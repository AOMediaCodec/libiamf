...
#define TAG2(a, b, c, d) (((a) << 24) | ((b) << 16) | ((c) << 8) | (d))
...
static uint32_t read_tag(struct wav_reader2* wr) {
  uint32_t tag = 0;
...
static uint32_t read_int32(struct wav_reader2* wr) {
  uint32_t value = 0;
...
static uint16_t read_int16(struct wav_reader2* wr) {
  uint16_t value = 0;
  value |= fgetc(wr->wav) << 0;
  value |= fgetc(wr->wav) << 8;
  return value;
...
void* wav_read_open2(const char* filename) {
  struct wav_reader2* wr = (struct wav_reader2*)malloc(sizeof(*wr));
  long data_pos = 0;
  ...
  while (1) {
    ...
    if (tag != TAG2('R', 'I', 'F', 'F') || length < 4) {
      fseek(wr->wav, length, SEEK_CUR);
      continue;
    }
    tag2 = read_tag(wr);
    length -= 4;
    if (tag2 != TAG2('W', 'A', 'V', 'E')) {
      fseek(wr->wav, length, SEEK_CUR);
      continue;
    }
...
void wav_read_close2(void* obj) {
  struct wav_reader2* wr = (struct wav_reader2*)obj;

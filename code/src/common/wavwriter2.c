...
static void write_string(struct wav_writer2* ww, const char* str) {
  fputc(str[0], ww->wav);
  fputc(str[1], ww->wav);
  fputc(str[2], ww->wav);
  fputc(str[3], ww->wav);
...
static void write_int32(struct wav_writer2* ww, int value) {
...
static void write_int16(struct wav_writer2* ww, int value) {
  fputc((value >> 0) & 0xff, ww->wav);
  fputc((value >> 8) & 0xff, ww->wav);
}

static void write_header(struct wav_writer2* ww, int length) {
...
int channels) { return (wav_write_open2(filename, WAVE_FORMAT_PCM, sample_rate,
bits_per_sample, channels));
}
*/
void* wav_write_open3(const char* filename, int format, int sample_rate,
                      int bits_per_sample, int channels) {
  struct wav_writer2* ww = (struct wav_writer2*)malloc(sizeof(*ww));
...
void wav_write_close2(void* obj) {
  struct wav_writer2* ww = (struct wav_writer2*)obj;
  if (ww->wav == NULL) {
    free(ww);
    return;
  }
